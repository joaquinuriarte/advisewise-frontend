...
const Semester = ({ classes, semester, semester_classes, selectedSemester, onSemesterSelection }) => {
  const classInfo = getClassInfo(semester_classes, classes);
  const totalCredits = classInfo.reduce((sum, info) => sum + info[1], 0);

  return (
    <div 
      className={`bg-white border rounded-xl w-80 p-2 m-2 ${semester.id === selectedSemester ? 'border-red' : ''}`} 
      onClick={() => onSemesterSelection(semester.id)}
    >
      ...
      <div className="p-2">
        ...
        {classInfo.map(([className, credits]) => (
          <div key={className} className="flex mb-1">
