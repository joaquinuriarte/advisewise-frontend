export async function fetchAllFourYearPlans(student_id) {
  try {
    const response = await fetch(`http://localhost:4000/schedule/allPlans/${student_id}`);
    if (!response.ok) {
      console.error('fetchAllFourYearPlans: Failed to fetch data from the backend');
...
export async function fetchAllClasses() {
  try {
    const response = await fetch('http://localhost:4000/schedule/allCourses');
    if (!response.ok) {
